...
public class FollowController implements CommunityConstant {
    ...
    public String getFollowees(@PathVariable("userId") int userId, Page page, Model model) {
        ...
        if (userList != null) {
            for (Map<String, Object> map : userList) {
                ...
                //判断正在查看的（别人的或者自己的）关注者 跟自己的关注关系是怎样的
    ...
    public String getFollowers(@PathVariable("userId") int userId, Page page, Model model) {
        ...
        if (userList != null) {
            for (Map<String, Object> map : userList) {
                User u = (User) map.get("user");
                //判断正在查看的（别人的或者自己的）粉丝跟自己的关注关系是怎样的
                map.put("hasFollowed", hasFollowed(u.getId()));
            }
        }
        model.addAttribute("users", userList);
    ...
    private boolean hasFollowed(int targetUserId) {
        ...
        int curUserId = hostHolder.getUser().getId();
        return followService.hasFollowed(curUserId, ENTITY_TYPE_USER, targetUserId);
    }
