...
public class LoginController implements CommunityConstant {
    ...
    public String activation(Model model, @PathVariable("userId") int userId, @PathVariable("code") String code) {
        ...
        if (result == ACTIVATION_SUCCESS) {
            model.addAttribute("msg", "激活成功,可使用此账号密码登录!");
    ...
    //这里应该使用post方式，否则会与登录页面冲突
    @RequestMapping(path = "/login", method = RequestMethod.POST)
    //需要将ticket保存到session中
    public String login(String username, String password, String code, boolean rememberme,
                        Model model, /*HttpSession session, */HttpServletResponse response,
                        //加了required = false，就可以在过期之后不会报错，而是取到空值
                        @CookieValue(value = "kaptchaOwner",required = false) String kaptchaOwner) {
        // 检查验证码
        //String kaptcha = (String) session.getAttribute("kaptcha");
        String kaptcha = null;
        if(StringUtils.isNotBlank(kaptchaOwner)){ // 如果还没失效，cookie中就有这个凭证
            String redisKey = RedisKeyUtil.generateKaptchaKey(kaptchaOwner);
